...
const transformedData = navigationMenuData.reduce((acc, item) => {
  if (item.isSection) {
    ...
    if (acc.length) {
      acc[acc.length - 1].items.push(newItem)
    } else {
      // Create a default section if the first item is not a section
      acc.push({ title: 'Navigation', items: [newItem] })
    }
  }
