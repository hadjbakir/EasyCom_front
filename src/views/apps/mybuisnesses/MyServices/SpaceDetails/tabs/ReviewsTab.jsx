...
const ReviewsTab = ({ space }) => {
  ...
  const handleReplySubmit = async reviewId => {
    if (!space?.id || !session?.user?.accessToken) {
    ...
    try {
      const response = await apiClient.post(
        `/workspaces/${space.id}/reviews/${reviewId}/reply`,
        { comment: replyText },
        {
          headers: {
        ...
        prevReviews.map(review =>
          review.id === reviewId
            ? {
                ...
                replyUser: {
                  name: session.user.fullName || 'Workspace Owner',
    ...
    <Card>
      <CardContent>
        ...
        <Box className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6'>
          <Typography variant='h6'>Workspace Reviews:</Typography>
              ...
              <Card key={review.id} sx={{ mb: 2, p: 3 }}>
                <Box className='flex gap-4'>
                  ...
                  <Box className='flex-1'>
                        ...
                        <Collapse in={expandedReplies[review.id]}>
                          ...
                          >
                            <Box className='flex gap-2'>
                              <Avatar
                                src={review.reply_user?.picture || '/images/avatars/default.png'}
                              ...
                              <Box>
                                <Box className='flex items-center gap-1'>
                                  <Typography variant='subtitle2'>
                                    {review.reply_user?.full_name || 'Workspace Owner'}
                                  </Typography>
                    ...
                    {/* Reply Form */}
                    {!review.reply && session?.user?.id === space?.user_id && (
                      <Box mt={2}>
                        <Button
                          variant='text'
                          size='small'
                        ...
                        >
                          {replyForms[review.id] ? 'Cancel Reply' : 'Reply'}
                        </Button>
